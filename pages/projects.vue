<template>
  <div class="space-y-6 max-w-4xl mx-auto mt-4 px-2 pb-10">
    ...
    <div v-else class="space-y-16">
      <div
        ...
        class="relative group scroll-mt-28"
      >
        ...
        <div class="flex flex-col md:flex-row gap-8">
          <div class="md:w-1/3" v-if="project.data.project_image?.url">
            <div class="border border-terminal-amber-dim border-opacity-40 p-1 bg-black/50">
              <img
                ...
                class="filter grayscale opacity-80 hover:grayscale-0 hover:opacity-100 transition-all duration-500 w-full object-cover"
                alt="Project Image"
              />
            </div>
          </div>
          ...
          <div class="md:w-2/3 space-y-5">
            ...
            <ul class="space-y-3 text-terminal-amber-dim text-sm md:text-base leading-relaxed text-justify">
              <li
                v-for="(desc, idx) in project.data.project_details"
                :key="idx"
                class="relative pl-4 before:content-['>'] before:absolute before:left-0 before:text-terminal-amber-dim before:opacity-50"
              >
                {{ desc.text }}
              </li>
            </ul>
            ...
            <div v-if="project.data.docs?.length > 0" class="mt-6 pt-4 border-t border-terminal-amber-dim border-opacity-30">
              ...
              <div class="flex flex-wrap gap-3">
                <a
                  v-for="(doc, idx) in project.data.docs"
                  :key="idx"
                  :href="doc.doc?.url"
                  target="_blank"
